...
def build_dataloader(config):
    ...
    transform = tvt.Compose([
        ...
        tvt.RandomResizedCrop(size=(config.input_dim, config.input_dim), antialias=False),
        tvt.ToDtype(torch.float32, scale=True),
 #       tvt.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])

    # Build dataset
    if not config.torchdata:
        if config.dataset_format == 'jpg':
            ...
            dataset = (
                ...
                .batched(config.batch_size))    # Do we actually need to batch here?
            dataloader_batch_size = None        # <---------------------|
        else:
            raise NotImplementedError("Unknown dataset format '%s'.." % config.dataset_format)
    else:
        if config.dataset_format == 'tar/jpg':
            from torchdata.datapipes.iter import FileLister

            def _preproc(item, cls_transform=int, jpg_transform=transform):
                _cls = cls_transform(item['.cls'].read().decode())
                _jpg = jpg_transform(Image.open(item['.jpg']).convert('RGB'))
                return _jpg, _cls

            files = glob.glob(config.input_channel + '/*.tar')
            
            dataset = FileLister(config.input_channel, recursive=True)
            if config.num_workers > 0:
                dataset = dataset.sharding_filter()
            dataset = dataset.open_files(mode='rb')
            dataset = dataset.load_from_tar()
            dataset = dataset.webdataset()
            dataset = dataset.map(_preproc)
            dataloader_batch_size = config.batch_size
        else:
            raise NotImplementedError("Unknown dataset format '%s'.." % config.dataset_format)
